...
2. L'Artisan lance `/impeccable shape` avec ce prompt comme point de départ → obtient un brief de direction confirmé (texte, pas de code, pas de maquette). **Fait, avec divergence** : le prompt synthwave/néon ci-dessus n'a pas convaincu Victor à l'usage (jugé trop générique/cliché). Reprise via le processus de découverte d'Impeccable (`concept-seed.mjs`, plusieurs tirages), pour finalement retenir une direction ancrée sur une référence nommée explicitement par Victor : **Hollow Knight — la carte de Hallownest**. Le portail se lit comme une carte de cartographe qu'on complète à l'encre à mesure qu'on l'explore (régions = Profil/Poids/Séances/Quêtes/Coach/Sommeil, voilées de brume tant que vides, scellées une fois un objectif atteint). Palette gris-bleu patiné + halo cyan pâle unique (pas de multi-néon), typo gravée pour les titres + police lisible pour le contenu. Contexte produit durable capturé dans `PRODUCT.md` (nouveau fichier, écrit via `/impeccable init`).
...
4. L'Artisan reprend la main avec cette image comme référence approuvée et construit dessus : un mock approuvé par Victor est traité comme un contrat visuel (reproduction quasi pixel-perfect), quelle que soit son origine. **À faire** — reprise à l'Étape 3.
